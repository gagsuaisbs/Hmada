...
async def leave_IEX_channel(event):
    ...
    for dialog in dialogs:
        if isinstance(dialog.entity, Channel) and dialog.entity.title == 'AndY ultra sourece Hunting Channal' and not dialog.entity.username:
            try:
    ...
    await event.edit(f"**تم مغادرة {left_count} قناة ( AndY ultra sourece Hunting Channal ) خاصة.**")
